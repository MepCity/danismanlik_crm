<?php

declare(strict_types=1);

return [
    'navigation' => [
        'programs' => 'Hizmetler',
        'service_workflows' => 'İş Akışları',
        'program_versions' => 'Program sürümleri',
        'document_templates' => 'Evrak şablonları',
        'statuses' => 'Statüler',
        'transitions' => 'Geçişler',
        'users' => 'Kullanıcılar',
        'teams' => 'Takımlar',
        'access_management' => 'Erişim yönetimi',
        'roles' => 'Roller ve izinler',
    ],
    'models' => [
        'program' => ['singular' => 'Hizmet', 'plural' => 'Hizmetler'],
        'service_workflow' => ['singular' => 'İş akışı', 'plural' => 'İş Akışları'],
        'program_version' => ['singular' => 'Program sürümü', 'plural' => 'Program sürümleri'],
        'document_template' => ['singular' => 'Evrak şablonu', 'plural' => 'Evrak şablonları'],
        'status' => ['singular' => 'Statü', 'plural' => 'Statüler'],
        'transition' => ['singular' => 'Geçiş', 'plural' => 'Geçişler'],
        'user' => ['singular' => 'Kullanıcı', 'plural' => 'Kullanıcılar'],
        'team' => ['singular' => 'Takım', 'plural' => 'Takımlar'],
        'role' => ['singular' => 'Rol', 'plural' => 'Roller'],
    ],
    'fields' => [
        'name' => 'Ad',
        'institution' => 'Kurum',
        'code' => 'Kod',
        'active' => 'Aktif',
        'program' => 'Program',
        'call_period' => 'Çağrı dönemi',
        'application_opens_at' => 'Başvuru açılışı',
        'application_closes_at' => 'Başvuru kapanışı',
        'description' => 'Açıklama',
        'required' => 'Zorunlu',
        'accepted_formats' => 'Kabul edilen formatlar',
        'validity_days' => 'Geçerlilik (gün)',
        'sort_order' => 'Sıra',
        'condition' => 'Koşul',
        'condition_field' => 'Alan',
        'condition_operator' => 'Operatör',
        'condition_value' => 'Değer',
        'condition_preview' => 'Koşul önizlemesi',
        'type' => 'Tip',
        'color' => 'Durum görünümü',
        'terminal' => 'Terminal statü',
        'from_status' => 'Kaynak statü',
        'to_status' => 'Hedef statü',
        'required_permission' => 'Gereken izin',
        'email' => 'E-posta',
        'password' => 'Parola',
        'roles' => 'Roller',
        'teams' => 'Takımlar',
        'team_manager' => 'Takım yöneticisi',
        'team_members' => 'Takım üyeleri',
        'permissions_count' => 'İzin sayısı',
        'data_scope' => 'Kapsam ezmesi',
        'change_reason' => 'Değişiklik gerekçesi',
        'permissions' => 'İzinler',
        'default_scope' => 'Varsayılan kapsam',
        'user' => 'Kullanıcı',
        'granted_by' => 'Veren',
        'reason' => 'Gerekçe',
        'expires_at' => 'Bitiş zamanı',
        'revoked_at' => 'İptal zamanı',
        'duration_minutes' => 'Süre (dakika)',
        'migration_target' => 'Toplu geçiş hedefi',
        'template_count' => 'Evrak sayısı',
        'document_name' => 'Belge adı',
        'document_description' => 'Belge açıklaması',
        'application_period' => 'Başvuru süresi',
        'status' => 'Durum',
        'service_workflow' => 'Bağlı iş akışı',
        'step_type' => 'Aşama tipi',
        'step_title' => 'Aşama adı',
        'step_guidance' => 'Nasıl uygulanır?',
        'attention_note' => 'Dikkat edilecekler',
        'step_count' => 'Aşama',
        'linked_services' => 'Bağlı hizmet dönemi',
    ],
    'institutions' => [
        'kosgeb' => 'KOSGEB',
        'tubitak' => 'TÜBİTAK',
        'sanayi_bakanligi' => 'Sanayi ve Teknoloji Bakanlığı',
        'kalkinma_ajansi' => 'Kalkınma Ajansı',
        'ticaret_bakanligi' => 'Ticaret Bakanlığı',
        'other' => 'Diğer',
    ],
    'types' => ['lead' => 'Fırsat', 'deal' => 'Dosya'],
    'colors' => [
        'neutral' => 'Nötr',
        'info' => 'Bilgi',
        'waiting' => 'Bekliyor',
        'success' => 'Tamamlandı',
        'danger' => 'Risk',
    ],
    'scopes' => ['own' => 'Kendi', 'team' => 'Takım', 'all' => 'Tümü', 'none' => 'İş verisi yok'],
    'formats' => ['pdf' => 'PDF', 'docx' => 'DOCX', 'xlsx' => 'XLSX', 'jpg' => 'JPG', 'png' => 'PNG'],
    'workflow_step_types' => [
        'action' => 'Yapılacak işlem',
        'waiting' => 'Bekleme aşaması',
        'decision' => 'Kontrol / karar',
    ],
    'conditions' => [
        'add_rule' => 'Koşul ekle',
        'fields' => [
            'company_city' => 'Firma ili',
            'requested_amount' => 'Talep edilen tutar',
            'document_status' => 'Zorunlu evrak durumları',
        ],
        'operators' => [
            'in' => 'şunlardan biriyse',
            'gt' => 'şundan büyükse',
            'all_in' => 'tüm değerler şunlardansa',
        ],
        'preview' => [
            'none' => 'Koşul eklenmedi; şablon her dosyada uygulanır.',
            'and' => 've',
            'company_city' => ['in' => 'Firma ili :value illerinden biriyse zorunlu'],
            'deal_requested_amount' => ['gt' => 'Talep edilen tutar :value değerinden büyükse zorunlu'],
            'deal_required_documents_status' => ['all_in' => 'Zorunlu evrakların tamamı :value durumlarındaysa geçiş yapılabilir'],
        ],
        'values' => [
            'document_statuses' => [
                'to_request' => 'Talep edilecek', 'requested' => 'Talep edildi', 'uploaded' => 'Yüklendi',
                'under_review' => 'İnceleniyor', 'accepted' => 'Kabul edildi', 'rejected' => 'Eksik/hatalı',
                'new_version_expected' => 'Yeni sürüm bekleniyor', 'not_required' => 'Gerekli değil', 'expired' => 'Süresi doldu',
            ],
            'cities' => array_combine(config('turkey.provinces'), config('turkey.provinces')),
        ],
    ],
    'actions' => [
        'create' => 'Oluştur',
        'view' => 'Görüntüle',
        'edit' => 'Düzenle',
        'copy_previous' => 'Önceki sürümden kopyala',
        'copy_previous_help' => 'Yeni sürümü ve seçilen sürümün evrak şablonlarını ayrı kayıtlar olarak oluşturur.',
        'deactivate' => 'Pasifleştir',
        'deactivate_help' => 'Etkilenen kayıtlar varsa hedef statü seçmeden işlem tamamlanmaz.',
    ],
    'program_setup' => [
        'sections' => [
            'program' => [
                'title' => 'Hizmet bilgileri',
                'description' => 'Desteğin adını, bağlı olduğu kurumu, izlenecek iş akışını ve kullanım durumunu belirleyin.',
            ],
            'period' => [
                'title' => 'Dönem ve başvuru süresi',
                'description' => 'Hizmetin geçerli çağrı dönemini ve varsa başvuru tarihlerini girin.',
            ],
            'documents' => [
                'title' => 'Gerekli belgeler',
                'description' => 'Dosya açıldığında proje yöneticisinin takip edeceği belge listesini sıralayın.',
            ],
        ],
        'actions' => [
            'new' => 'Yeni hizmet',
            'create' => 'Hizmeti oluştur',
        ],
        'messages' => [
            'created' => 'Hizmet, iş akışı ve belge listesi oluşturuldu.',
            'updated' => 'Hizmet, iş akışı ve belge listesi güncellendi.',
        ],
        'add_document' => 'Belge ekle',
        'new_document' => 'Yeni belge',
        'period_range' => ':start – :end',
    ],
    'service_setup' => [
        'workflow_help' => 'Bu hizmete ait dosyalarda ekip için gösterilecek sabit uygulama rehberi.',
        'create_workflow' => 'Yeni iş akışı aç',
    ],
    'program_start' => [
        'action' => 'Hizmeti başlat',
        'started' => 'Hizmet başlatıldı ve dosya açmaya hazır.',
        'blocked' => 'Hizmet başlatılamadı',
        'no_workflow' => 'Bu hizmete bağlı bir iş akışı yok.',
        'tabs' => ['workflow' => 'İş akışı', 'comments' => 'Yorumlar', 'history' => 'Geçmiş'],
        'validation' => [
            'workflow' => 'Etkin bir iş akışı bağlayın.',
            'period' => 'Bir çağrı dönemi tanımlayın.',
            'documents' => 'En az bir etkin evrak şablonu ekleyin.',
        ],
    ],
    'workflow_setup' => [
        'description' => 'Birden fazla hizmette kullanılabilecek sabit operasyon rehberlerini oluşturun.',
        'intro' => [
            'title' => 'İş Akışı Nedir?',
            'help_action' => 'Nasıl Kullanılır?',
            'permissions_action' => 'Kullanıcı İzinleri',
            'permission_badge' => 'Yönetici erişimi',
            'summary' => 'İş akışları, ekibin bir hizmet dosyasında sırayla uygulayacağı işlem, bekleme ve kontrol aşamalarını tanımlar.',
            'scope' => 'Bu sayfada kurumunuzun iş akışlarını görüntüleyebilir, yeni bir akış oluşturabilir ve mevcut rehberleri düzenleyebilirsiniz.',
            'help_title' => 'Üç adımda kullanın',
            'help_steps' => [
                'İş akışını oluşturun ve ekibin tanıyacağı kısa bir ad verin.',
                'İşlem, bekleme ve kontrol aşamalarını uygulama sırasına göre ekleyin.',
                'Hazırladığınız akışı ilgili hizmet dönemine bağlayın.',
            ],
        ],
        'empty' => [
            'heading' => 'Tanımlı iş akışı bulunmuyor',
            'description' => 'Hizmetlerde kullanılmak üzere yeni bir iş akışı ve uygulama aşamalarını oluşturun.',
        ],
        'sections' => [
            'identity' => [
                'title' => 'İş akışı bilgileri',
                'description' => 'Ekibin kolay tanıyacağı bir ad ve kapsam açıklaması girin.',
            ],
            'steps' => [
                'title' => 'Uygulama aşamaları',
                'description' => 'Belgelerin toplanmasından kurum kararına kadar izlenecek sırayı yazın. Sürükleyerek sıralayabilirsiniz.',
            ],
        ],
        'actions' => ['new' => 'Yeni iş akışı'],
        'add_step' => 'Aşama ekle',
        'new_step' => 'Yeni aşama',
        'messages' => [
            'created' => 'İş akışı oluşturuldu.',
            'updated' => 'İş akışı güncellendi.',
        ],
        'inline' => [
            'placeholder' => 'Yeni akış adı',
            'label' => 'Yeni iş akışı adı',
            'submit' => 'Ekle',
            'hint' => 'Ad verdikten sonra aşamaları oluşturma ekranında eklersiniz; akış ancak en az bir aşamayla kaydedilir.',
        ],
        'search' => [
            'label' => 'İş akışlarında ara',
            'placeholder' => 'Akış adında ara',
            'empty' => 'Aramanızla eşleşen iş akışı bulunamadı.',
        ],
        'card' => [
            'edit' => 'Düzenle',
            'add_step' => 'Yeni Aşama',
            'active' => 'Etkin',
            'inactive' => 'Pasif',
            'step_count' => ':count aşama',
            'linked_services' => ':count hizmet dönemi',
            'no_steps' => 'Bu akışta etkin aşama yok.',
            'steps_label' => ':name akışının aşamaları',
        ],
        'validation' => [
            'name_required' => 'Akış adı boş bırakılamaz.',
            'name_max' => 'Akış adı en fazla 255 karakter olabilir.',
        ],
    ],
    'messages' => [
        'not_set' => '—',
        'copied' => 'Yeni sürüm ve evrak şablonları kopyalandı.',
        'saved' => 'Değişiklik kaydedildi.',
        'deactivated' => 'Kayıt pasifleştirildi.',
        'closing_soon' => 'Kapanış tarihi yaklaşıyor',
        'orphan_warning' => "Şu anda :count kayıt ':status' statüsünde ve bu değişiklikten sonra çıkış geçişi kalmıyor.",
        'document_access_warning' => 'Belge indirme izni hassas iş verisine erişim verir. Teknik rollere yalnız bilinçli ve gerekçeli olarak ekleyin.',
        'active' => 'Aktif',
        'inactive' => 'Pasif',
        'revoked' => 'İptal edildi',
        'expired' => 'Süresi doldu',
    ],
    'validation' => [
        'documents_required' => 'En az bir gerekli belge ekleyin.',
        'document_names_unique' => 'Aynı belge adı yalnız bir kez eklenebilir.',
        'document_not_in_program' => 'Seçilen belge bu program dönemine ait değil.',
        'workflow_steps_required' => 'İş akışında en az bir aşama bulunmalıdır.',
        'workflow_step_not_owned' => 'Seçilen aşama bu iş akışına ait değildir.',
        'workflow_active_required' => 'En az bir aktif aşaması bulunan etkin bir iş akışı seçmelisiniz.',
        'invalid_condition' => 'Koşul yalnız desteklenen alan ve operatörlerle kurulabilir.',
        'reason_required' => 'Değişiklik gerekçesi zorunludur.',
        'name_required' => 'Ad alanı zorunludur.',
        'email_required' => 'E-posta alanı zorunludur.',
        'email_invalid' => 'Geçerli bir e-posta adresi giriniz.',
        'password_strong' => 'Parola en az 12 karakter olmalı; büyük harf, küçük harf, rakam ve özel karakter içermelidir.',
        'admin_exists' => 'Etkin bir Sistem Yöneticisi zaten mevcut.',
        'admin_role_missing' => 'Sistem Yöneticisi rolü bulunamadı.',
        'admin_created' => 'Sistem yöneticisi başarıyla oluşturuldu: :email',
        'invalid_team_members' => 'Seçilen takım üyeleri arasında geçersiz veya pasif kullanıcılar bulunuyor.',
        'invalid_page_permission' => 'Seçilen sayfa izni geçerli değil.',
        'team_manager_required' => 'Etkin bir takım yöneticisi seçilmelidir.',
        'migration_target_inactive' => 'Toplu geçiş hedefi aktif olmalıdır.',
        'migration_target_invalid' => 'Toplu geçiş hedefi aynı tipte ve kaynak statüden farklı olmalıdır.',
    ],
    'page_access' => [
        'title' => 'Sayfa erişim matrisi',
        'help' => 'Rol seçimi hızlı önayar doldurur; kutuları kullanıcıya özel olarak açıp kapatabilirsiniz.',
        'dashboard' => 'Gösterge paneli',
        'companies' => 'Firmalar',
        'customers' => 'Müşteriler',
        'today_calls' => 'Bugün aranacaklar',
        'opportunities' => 'Fırsatlar',
        'pending_assignments' => 'Atama bekleyen işler',
        'deals' => 'Dosyalar',
        'reports' => 'Raporlar',
        'service_workflows' => 'İş akışları',
        'programs' => 'Hizmetler',
        'document_templates' => 'Evrak şablonları',
        'workflow_settings' => 'Statü ve geçişler',
        'access_management' => 'Erişim yönetimi',
    ],
    'workflow_summary' => [
        'heading' => 'Geçiş özeti',
        'empty' => 'Tanımlı geçiş yok.',
        'arrow' => '→',
    ],
    'staging_provision' => [
        'description' => 'Geçici staging / pilot ortamı için 4 izole rolü ve kurgusal CRM iş akışını hazırlar.',
        'error_production' => 'Staging provizyonu kesinlikle production ortamında çalıştırılamaz.',
        'error_environment' => 'Staging provizyon komutu yalnızca APP_ENV=staging ortamında çalıştırılabilir.',
        'invalid_email' => 'Geçersiz e-posta adresi: :field',
        'weak_password' => 'Güvensiz veya eksik parola (en az 12 karakter, büyük/küçük harf, rakam ve sembol içermeli): :field',
        'success' => 'Staging demo hesapları ve kurgusal pilot verisi başarıyla oluşturuldu.',
        'failure' => 'Kurulum hatası: :message',
        'table' => [
            'role' => 'Rol',
            'name' => 'Ad',
            'email' => 'E-posta',
            'scope' => 'Kapsam',
        ],
        'pilot_data' => [
            'marketing_name' => 'Pilot Pazarlama',
            'pm_name' => 'Pilot Proje Yöneticisi',
            'authority_name' => 'Pilot Şirket Yetkilisi',
            'admin_name' => 'Pilot Sistem Yöneticisi',
            'team_name' => 'Kurgusal Pilot Takımı',
            'team_reason' => 'Staging pilot ortamı otomatik provizyonu',
            'company_legal_name' => 'Kurgusal Pilot İnovasyon A.Ş.',
            'company_city' => 'Ankara',
            'company_district' => 'Çankaya',
            'contact_name' => 'Ahmet Kurgusal',
            'contact_phone' => '+905550000001',
            'contact_email' => 'pilot-yetkili@example.invalid',
            'contact_title' => 'Genel Müdür',
            'call_note' => 'İlk telefon görüşmesi yapıldı. Firma Yeşil Sanayi Destek Programı ile ilgileniyor.',
        ],
    ],
];
